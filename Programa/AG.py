...
def genetic_algorithm(objective_func, g, h, lower_bound, upper_bound,
                      ...
                      mutation_prob=MUTATION_PROB, eta_mut=ETA_MUT, lam=LAMBDA, secondary_func=None):
    """
    ...
    best_x_history = []
    constraint_violations_history = []
    secondary_fitness_history = []  # Nueva lista para el fitness secundario
    ...
    for gen in range(num_generations):
        ...
        # Calcular valor de la función secundaria para el mejor individuo
        if secondary_func:
            secondary_fitness = secondary_func(elite)
            secondary_fitness_history.append(secondary_fitness)
        ...
        new_fitness = np.array(evaluate_individuals_with_constraints(new_population, objective_func, g, h, lam))
